...
export function PointCloudFusionModal({ frames, onClose }: PointCloudFusionModalProps) {
  ...
  const handleFusePointClouds = () => {
    if (selectedFrames.length === 0) {
      setError(t('leakDetection.cloudPointConfiguration.selectAtLeastOnePointCloud'));
      return;
    }
    setError(null);
    ...
    <div className="w-full max-w-2xl bg-white/70 dark:bg-slate-950/60 rounded-2xl border border-slate-200/70 dark:border-white/10 shadow-2xl backdrop-blur-xl overflow-hidden transition-all duration-200 max-h-[90vh] flex flex-col">
      <div className="flex items-center justify-between p-6 border-b border-slate-200/50 dark:border-white/5">
        <div className="min-w-0">
          <h2 className="text-lg font-semibold text-slate-900 dark:text-slate-100">
            {t('leakDetection.cloudPointConfiguration.fusionModalTitle')}
          </h2>
          <p className="text-xs text-slate-500 dark:text-slate-500">
            {t('leakDetection.cloudPointConfiguration.fusionModalDescription')}
          </p>
        </div>
        <button
          onClick={onClose}
      ...
      <div className="p-6 pb-4">
        <CadViewer
          ...
          clip={null}
          emptyMessage={t('leakDetection.cloudPointConfiguration.fusionPreviewEmpty')}
        />
      </div>
      ...
      <ScrollableContainer className="flex-1 px-6 pb-6">
        {frames.length === 0 ? (
          <p className="text-sm text-slate-500 dark:text-slate-400">
            {t('leakDetection.cloudPointConfiguration.noPointCloudsAvailable')}
          </p>
        ) : (
        ...
        {fusedPointCount !== null && (
          <p className="mt-3 text-xs text-slate-500 dark:text-slate-500">
            {fusedPointCount.toLocaleString()} {t('leakDetection.cloudPointConfiguration.fusedPointsSummary')}
          </p>
        )}
      </ScrollableContainer>
      ...
      <div className="border-t border-slate-200/50 dark:border-white/5 p-4 bg-white/30 dark:bg-slate-900/20 flex items-center justify-between gap-3">
        <div className="flex items-center gap-3">
          ...
          >
            ...
            {t('leakDetection.cloudPointConfiguration.fusePointClouds')}
          </button>
          ...
          >
            ...
            {t('leakDetection.cloudPointConfiguration.export')}
          </button>
        </div>
        ...
        >
          {t('leakDetection.cloudPointConfiguration.done')}
        </button>
      </div>
    </div>
  );
