        ...
        <div className="space-y-4">
          <div className={pointCloudGeometry ? 'grid grid-cols-2 gap-3' : 'grid grid-cols-1'}>
            <div className="space-y-1.5">
              ...
              >
                {t('leakDetection.cloudPointConfiguration.voxelSizeLabel')}
              </label>
              <input
                id="cad-voxel-size"
                type="number"
            ...
            {pointCloudGeometry && (
              <div className="space-y-1.5">
                <label
                  ...
                  className="block text-xs font-medium text-slate-600 dark:text-slate-400"
                >
                  {t('leakDetection.cloudPointConfiguration.exportScaleLabel')}
                </label>
                <input
                  id="cad-export-scale"
                  type="number"
          ...
          {pointCloudGeometry && (
            <p className="text-xs text-slate-500 dark:text-slate-500">
              ...
              {t('leakDetection.cloudPointConfiguration.pointsGenerated')}
            </p>
          )}
          ...
          {pointCloudGeometry && clipRanges && clip && (
            <div className="space-y-3 p-3 rounded-lg border border-slate-200/60 dark:border-white/10 bg-white/40 dark:bg-slate-900/30">
              <p className="text-xs font-semibold text-slate-700 dark:text-slate-300">
                {t('leakDetection.cloudPointConfiguration.clipTitle')}
              </p>
              {CLIP_AXES.map((axis) => (
                <ClipAxisControl
                  key={axis}
                  axisLabel={axis}
                  minLabel={t('leakDetection.cloudPointConfiguration.clipMin')}
                  maxLabel={t('leakDetection.cloudPointConfiguration.clipMax')}
                  value={clip[axis]}
        ...
        <div className="flex items-center gap-3">
          ...
          {pointCloudGeometry && (
            ...
            >
              ...
              {t('leakDetection.cloudPointConfiguration.exportPly')}
            </button>
          )}
        </div>
