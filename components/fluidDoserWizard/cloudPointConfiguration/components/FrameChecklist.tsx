    ...
    <div className="grid grid-cols-2 sm:grid-cols-3 xl:grid-cols-4 gap-2">
      {frames.length === 0 && missingFiles.length === 0 && !targetGroups?.length && (
        <p className="col-span-full text-sm text-slate-500 dark:text-slate-400">
          {t('fluidDoser.cloudPointConfiguration.noFrameLoaded')}
        </p>
      )}
            ...
            <label className="flex items-start gap-3 cursor-pointer">
              ...
              <span className="flex-1 min-w-0">
                ...
                <span className="block text-xs text-slate-500 dark:text-slate-500">
                  {transform
                    ? `${t('fluidDoser.pointCloud.fitness')} ${transform.fitness.toFixed(4)} · ${t(
                        'fluidDoser.pointCloud.rmse'
                      )} ${transform.rmse.toFixed(4)}`
                    : t('fluidDoser.cloudPointConfiguration.noRansacTransform')}
                </span>
                {target.missingFiles.length > 0 && (
                  <span className="block text-[10px] text-amber-600 dark:text-amber-500 mt-0.5">
                    {t('fluidDoser.cloudPointConfiguration.missingFiles')} {target.missingFiles.join(', ')}
                  </span>
                )}
              </span>
            </label>
      ...
      >
        ...
        {t('fluidDoser.cloudPointConfiguration.addPointCloud')}
      </button>
      ...
      >
        ...
        {t('fluidDoser.cloudPointConfiguration.pointCloudFusion')}
      </button>
      ...
      {missingFiles.length > 0 && (
        <div className="col-span-full mt-1 p-3 rounded-lg border border-amber-300/50 dark:border-amber-500/30 bg-amber-50/50 dark:bg-amber-950/20">
          <p className="text-xs font-semibold text-amber-700 dark:text-amber-400 mb-1">
            {t('fluidDoser.cloudPointConfiguration.missingFiles')}
          </p>
