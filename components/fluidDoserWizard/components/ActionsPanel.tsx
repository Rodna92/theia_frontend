...
import { FluidDoserSystemStatus } from '@/client/types/fluidDoser';

// Define the logic states explicitly
export type ProcessState = 'idle' | 'running' | 'finished';

interface ActionsPanelProps {
  systemStatus?: FluidDoserSystemStatus[];
  processState: ProcessState; // Controlled by parent
  onStart?: () => void;
  onTest?: () => void;
  onStop?: () => void;
  onReset?: () => void;
    ...
    return (
      <div className="flex flex-col gap-3">
        ...
        <div className="flex items-center justify-center gap-2 p-3 rounded-lg border border-sky-200 bg-sky-50 text-sky-700 dark:bg-sky-900/20 dark:border-sky-800 dark:text-sky-300 font-medium transition-all cursor-default">
          ...
          <span>{t('fluidDoser.actions.running') || 'Processing...'}</span>
        </div>

        {/* Stop Button */}
        <SetButton
          onClick={onStop}
          label={t('fluidDoser.actions.stop') || 'Stop'}
  ...
  if (processState === 'finished') {
    return (
      <div className="flex flex-col gap-3">
        <SetButton
          onClick={onReset}
          label={t('fluidDoser.actions.reset') || 'Reset Process'}
  ...
  return (
    <>
      <div className="flex flex-col gap-3">
        <StartButton
          onClick={onStart}
          label={t('fluidDoser.actions.start')}
        />

        <SetButton
          onClick={onTest}
          label={t('fluidDoser.actions.test')}
        ...
        <SetButton
          ...
          label={t('fluidDoser.actions.configure')}
